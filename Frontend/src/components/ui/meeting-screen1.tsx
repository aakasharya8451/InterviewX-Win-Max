...
import { Settings, Mic, MicOff, Video, VideoOff, PhoneOff } from "lucide-react"
...
export default function MeetingScreen({ stream, userName, onEndCall }: MeetingScreenProps) {
  ...
  // Connect output WebSocket - exact logic from original
  const connectOut = () => {
  ...
  // Initialize audio input WebSocket - with reconnection logic
  const initAudioIn = () => {
    ...
    audioInSocketRef.current.addEventListener("close", () => {
      console.log("[WS-In] Closed")
      cleanupIn()
      // Add reconnection logic for input socket
      setConnectionStatus("connecting")
  ...
  const startMicCapture = () => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        processorInRef.current = audioContextInRef.current!.createScriptProcessor(4096, 1, 1)
        ...
        processorInRef.current.onaudioprocess = (e) => {
          ...
          if (!isMicMutedRef.current && audioInSocketRef.current?.readyState === WebSocket.OPEN) {
  ...
  // Updated cleanup logic - don't close socket if reconnecting
  const cleanupIn = () => {
    if (processorInRef.current) processorInRef.current.disconnect()
    if (audioContextInRef.current) audioContextInRef.current.close()
    // Don't close socket here as it will reconnect
      ...
      try {
        if (!stream) {
          await navigator.mediaDevices
            ...
            .catch(() => console.log("Permission denied, but continuing to enumerate devices"))
        }
            ...
            <div className="relative w-full h-full flex flex-col items-center justify-center">
              ...
              <div className="text-center z-10">
                <div className="relative mb-6">
                  ...
                  >
                    <div
                      className={`w-2 h-2 md:w-3 md:h-3 bg-white rounded-full ${isRemoteAudioPlaying ? "animate-pulse" : ""}`}
                    ></div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="inline-flex items-center gap-3 px-4 py-2 md:px-6 md:py-3 bg-black/40 backdrop-blur-sm rounded-full border border-white/20">
                    <span className="text-lg md:text-xl font-medium text-white">{remoteName}</span>
                  </div>

                  <div className="flex items-center justify-center gap-2 text-sm">
                    <div className={`w-2 h-2 ${getConnectionColor()} rounded-full animate-pulse`}></div>
                    <span
      ...
      <div className="absolute bottom-16 right-4 md:bottom-24 md:right-6 w-[160px] h-[100px] md:w-[220px] md:h-[140px] group">
        <div className="w-full h-full bg-gradient-to-br from-[#1e2a3a] to-[#2a3441] rounded-xl overflow-hidden shadow-2xl border border-white/20 backdrop-blur-sm relative transition-all duration-300 hover:scale-105">
          {isVideoEnabled && stream ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-green-500/20 to-green-700/20">
              <Avatar className="w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br from-green-500 to-green-700 shadow-lg">
                <AvatarFallback className="text-xl font-semibold text-white">{userInitials}</AvatarFallback>
              </Avatar>
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
            <div className="flex gap-2">
              <Button
                size="sm"
                variant="ghost"
                className={`h-8 w-8 p-0 text-white hover:bg-white/20 ${!isVideoEnabled ? "bg-red-500/80" : ""}`}
              ...
              <Button
                size="sm"
                variant="ghost"
                className={`h-8 w-8 p-0 text-white hover:bg-white/20 ${!isAudioEnabled ? "bg-red-500/80" : ""}`}
          ...
          <div className="absolute top-2 left-2 flex items-center gap-1">
            <div
              className={`w-2 h-2 ${isAudioEnabled ? "bg-green-400" : "bg-red-400"} rounded-full animate-pulse`}
            ></div>
      ...
      <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        <DialogContent className="max-w-sm md:max-w-md bg-[#1a2235]/95 backdrop-blur-xl border border-white/20">
          ...
          <div className="space-y-6 py-4">
            <div>
              <h3 className="text-sm font-medium mb-3 text-white">Audio Input</h3>
              {audioInputDevices.length > 0 ? (
                ...
                >
                  {audioInputDevices.map((device) => (
                    <option key={device.deviceId} value={device.deviceId} className="bg-[#2a3441]">
            ...
            <div>
              <h3 className="text-sm font-medium mb-3 text-white">Audio Output</h3>
              ...
              {audioOutputDevices.length > 0 && !("setSinkId" in HTMLMediaElement.prototype) && (
                <p className="text-xs text-yellow-400 mt-2">
                  Note: Your browser may not support changing audio output devices
                </p>
              )}
            </div>
          </div>
